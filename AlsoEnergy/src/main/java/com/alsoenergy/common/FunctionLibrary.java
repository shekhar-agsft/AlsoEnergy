...
public class FunctionLibrary 
{
	...
	public boolean isElementPresent(List<WebElement> slocators) {
		driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
		try {
			logger.info("no of webelement"+slocators.size());
			driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
			for(WebElement element:slocators)
			{
			...
			e.printStackTrace();
			driver.manage().timeouts().implicitlyWait(10, TimeUnit.SECONDS);
			logger.info(slocators + " is not displayed");
		}
